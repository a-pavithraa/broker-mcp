...
public class BreezeApiClient {
    ...
    public SessionDetails generateSession(String apiKey, String apiSession) {
        ...
        if (success.isMissingNode() || !success.has("session_token")) {
            ...
            throw new BreezeApiException("Session generation failed: " + error);
        }
    ...
    private JsonNode executeRequest(HttpRequest request) {
        ...
        try {
            ...
            throw new BreezeApiException("Request interrupted", e);
        } finally {
            requestLock.unlock();
        }
    }
    ...
    private JsonNode executeWithRetry(HttpRequest request) {
        ...
        while (true) {
            try {
                ...
                throw new BreezeApiException("Failed to parse Breeze API response: " + e.getMessage(), e);
            } catch (IOException e) {
                throw new BreezeApiException("Failed to communicate with Breeze API: " + e.getMessage(), e);
            } catch (InterruptedException e) {
                Thread.currentThread().interrupt();
                throw new BreezeApiException("Request interrupted", e);
            }
    ...
    private String toJson(Object body) {
        try {
            ...
            throw new BreezeApiException("Failed to serialize request body", e);
        }
    }
}
