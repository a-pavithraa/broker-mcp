...
public class CompositeBrokerGateway implements BrokerDataProvider {
    ...
    public QuoteSnapshot getQuoteForBroker(String stockCode, String exchangeCode, String productType, String broker) {
    ...
    public OrderRouting resolveOrderRouting(String stockCode, String isin, String action, String requestedBroker) {
        ...
        if (iciciAvailable && zerodhaAvailable) {
            throw new BrokerApiException("Sell holding exists on both brokers. Specify broker='icici' or broker='zerodha'.");
        }
        if (iciciAvailable) {
    ...
    private <T> T marketDataCall(BrokerCall<T> call, String label, Map<String, ?> context) {
        ...
        try {
            ...
            return result;
        } catch (BrokerCapabilityException | BrokerApiException ex) {
    ...
    public record OrderRouting(String broker, HoldingSnapshot holding) {
    }
}
